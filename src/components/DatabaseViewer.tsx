...
export default function DatabaseViewer() {
  ...
  const handleClearTable = async () => {
    if (confirm(`Clear all data in ${selectedTable}?`)) {
  ...
  return (
    <div className={`p-4 md:p-6 h-screen flex flex-col ${isDark ? "bg-gray-900" : "bg-gray-50"}`}>
      <Card className={`flex-1 flex flex-col shadow-lg ${isDark ? "bg-gray-800" : "bg-white"}`}>
        <CardHeader className={`flex flex-col md:flex-row md:justify-between md:items-center gap-4 pb-4 border-b-2 ${isDark ? "border-gray-700" : "border-gray-200"}`}>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Button color="danger" variant="flat" size="sm" onPress={handleClearTable} className="font-semibold relative" startContent={<FlowbiteIcon name="trash" size={16} />}>
              Clear
            </Button>
          </div>
        </CardHeader>

        <CardBody className="space-y-4 flex-1 overflow-hidden flex flex-col">
          {/* Table Tabs */}
                    ...
                    <Card key={index} className={`shadow-sm ${isDark ? "bg-gray-800" : "bg-white"}`} style={{minWidth: 0, border: 'none', boxShadow: '0 2px 8px 0 rgba(0,0,0,0.04)'}}>
                      <CardBody className="p-2 space-y-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`inline-flex items-center text-white font-bold text-sm tracking-wide px-3 py-1 rounded-full shadow ${isDark ? 'bg-gradient-to-r from-blue-700 to-blue-500 border-blue-900' : 'bg-gradient-to-r from-blue-400 to-blue-500 border-blue-500'}`}
                            style={{opacity: 1, minWidth: 0, maxWidth: '100%', textShadow: '0 1px 2px rgba(0,0,0,0.10)'}}
                          >
                            {mainKey}
                          </span>
